...
trial{
	...
	stimulus_event{
		...
		} InstructionCue_stim; #Instruction Cue
	stimulus_event{
		picture { bitmap "ic5"; x = 0; y = 0; } instructioncue_blank;
		time = 200;
		duration = 800;
	#	response_active = true;
		} InstructionCueBlank; #InstructionCue Blank
...
array<int> sequence_array[6] = {1,3,2,3,4,2}; #Sequence 
array<string> block_array[5] = {"R", "S", "S", "S", "R"}; #Order of Trials
...
loop int p = 1 until p > 3
	...
	loop int i = 1 until i > 5
		...
		if block_array[i] == "R" then
			...
			output.print("\n");
			output.print("\n");
			loop int k = 1 until k > 18 #Number of trials for Random task
				begin 
				output.print( "Trial ");
				output.print( k );
				output.print("\n");
			...
			output.print("\n");
			S_trial.present();
			loop int j = 1 until j > 6
				begin
				loop int m = 1 until m > 6
					begin
					output.print( "Trial ");
					output.print( m * j );
					output.print("\n");
